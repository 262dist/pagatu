...
`id_orden` sí es una llave foránea normal (`REFERENCES ordenes(id)`), porque `ordenes` y `orden_detalles` viven en la misma base de datos de `pagatu-orden-ms`. `id_producto` e `id_cliente`, en cambio, **no** llevan `REFERENCES` — el producto vive en la base de datos de `pagatu-catalogo-ms` y el cliente en la de `pagatu-cliente-ms` (otro microservicio, con su propia base de datos), cada uno un microservicio distinto; validar que existan es responsabilidad del código (una consulta HTTP al microservicio correspondiente), no de una llave foránea entre bases de datos separadas.

`id_cliente` es `NULL`-able, no obligatorio: mientras el único cliente sea el cliente de prueba (mostrador, sin autoservicio web), una orden puede registrarse sin identificar a nadie; recién cuando exista autoservicio real (S11) ese valor pasa a ser obligatorio en el punto donde el propio cliente autenticado crea su orden (razonamiento completo en S6, 3.3). `nombre_cliente`/`direccion_cliente` quedan declaradas desde ahora, por la misma razón que el resto de columnas de `ordenes` — una orden es un documento histórico —, pero no se llenan todavía: quedan en `NULL` hasta que `pagatu-orden-ms` consulte a `pagatu-cliente-ms` por Feign, el mismo patrón que ya copia `nombre_producto`/`precio_unitario` desde `pagatu-catalogo-ms` (S6, 3.12-3.13).

`precio_unitario` y `nombre_producto` en `orden_detalles`, igual que `total` en `ordenes`, aceptan `NULL`: una línea (o una orden completa) que no pudo validarse contra `pagatu-catalogo-ms` queda registrada de todas formas, sin esos datos confirmados, en vez de no registrarse en absoluto — el mismo criterio que protege el Circuit Breaker de S6 (3.3, 3.17). `expira_en` queda declarada para el futuro carrito con plazo de vencimiento (S6, 3.3-3.4), sin poblarse todavía ni tener un proceso que la revise — eso se retoma cuando exista autoservicio real (S11).
