...
Los pasos 3.2-3.14 de abajo se enfocan solo en `pagatu-config` y en migrar `pagatu-catalogo-ms` (3.1 solo verifica `pagatu-catalogo-ms`) — no avanzan a construir `pagatu-orden-ms` ni `pagatu-cliente-ms`. Si en la sección 4 (trabajo autónomo) todavía no tienes esos dos microservicios creados, revisa [Alcance por microservicio y proyecto base](../proyecto-sello/alcance-microservicios.md) — sigue exactamente el mismo patrón de `pagatu-catalogo-ms` (S1).
...
`pagatu-orden-ms`/`pagatu-cliente-ms` ([Alcance por microservicio y proyecto base](../proyecto-sello/alcance-microservicios.md)) **no** son prerrequisito de los pasos 3.2-3.14: esos solo trabajan sobre `pagatu-config` y `pagatu-catalogo-ms`. Esta sesión, tal como se dictó, no avanzó a construirlos — quedan como preparación para la actividad autónoma (sección 4), no como algo ya resuelto en clase.
...
**Error frecuente**: `docker compose logs pagatu-catalogo-ms` muestra `Application run failed` con `FlywayValidateException: Migration checksum mismatch for migration version 1`. Significa que el volumen de PostgreSQL de PROD local ya tiene aplicada una versión anterior de `V1__create_catalogo_tables.sql`, distinta a la que existe hoy en el proyecto (S1 ya advirtió esto: no se edita una migración ya aplicada, se crea una `V2` — pero si el volumen quedó con una versión vieja de todos modos, hay que empezar de cero). Como este entorno de PROD local es de práctica, sin datos reales que proteger, la solución es bajar el entorno **con** el volumen y volver a levantarlo:
...
`down` sin `-v` borra los contenedores pero no el volumen — el historial de Flyway (`flyway_schema_history`) sigue ahí y el error se repite igual.
...
```bash
docker compose logs --tail=80 pagatu-config
```
